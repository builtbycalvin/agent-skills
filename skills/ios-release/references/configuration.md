...
output such as `build`, `dist`, or `DerivedData`. App keys, display names, and
aliases are case-insensitive selectors and must not collide. Group IDs and names
must be explicit selectors. Resolve live identity through the owning ASC skills
before every remote effect.
