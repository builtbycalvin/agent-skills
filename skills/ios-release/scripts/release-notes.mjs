...
function duplicateTopLevelJsonKeys(raw) { const seen = new Set(); const duplicates = new Set(); let depth = 0; let stringStart = -1; let escaped = false; for (let index = 0; index < raw.length; index += 1) { const character = raw[index]; if (stringStart >= 0) { if (escaped) escaped = false; else if (character === '\\') escaped = true; else if (character === '"') { let next = index + 1; while (/\s/.test(raw[next] ?? '')) next += 1; if (depth === 1 && raw[next] === ':') { const key = JSON.parse(raw.slice(stringStart, index + 1)); if (seen.has(key)) duplicates.add(key); else seen.add(key); } stringStart = -1; } } else if (character === '"') stringStart = index; else if (character === '{' || character === '[') depth += 1; else if (character === '}' || character === ']') depth -= 1; } return [...duplicates]; }
...
export function parseReleaseNote(markdown, source = '<string>') {
  ...
  if (lines[0] !== '---') errors.push(`${source}: front matter must start with ---`); else { const end = lines.indexOf('---', 1); if (end < 0) errors.push(`${source}: front matter is not closed`); else { const raw = lines.slice(1, end).join('\n'); try { frontmatter = JSON.parse(raw); frontmatterParsed = true; for (const key of duplicateTopLevelJsonKeys(raw)) errors.push(`${source}: duplicate front matter key ${JSON.stringify(key)}`); } catch (error) { errors.push(`${source}: front matter is not valid JSON: ${error.message}`); } body = lines.slice(end + 1).join('\n'); } }
