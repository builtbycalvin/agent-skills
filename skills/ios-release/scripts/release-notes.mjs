...
function committedFileBlob(repo, commit, relative) { try { return runGit(repo, ['rev-parse', '--verify', '--end-of-options', `${commit}:${relative}`]); } catch { return null; } }
function committedFileMatches(repo, commit, relative, content, committedBlob = committedFileBlob(repo, commit, relative)) { if (!committedBlob) return false; try { const worktreeBlob = execFileSync('git', ['-C', repo, 'hash-object', `--path=${relative}`, '--stdin'], { input: content, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }).trim(); return committedBlob === worktreeBlob; } catch { return false; } }
function validateGitEvidence(repo, requestedSourceCommit, frontmatter, archiveRelative, markdown, reasons) {
  ...
  if (!requested) addReason(reasons, 'requested-source-commit-unresolved', requestedSourceCommit);
    ...
    if (requested === source) {
      const committedBlob = committedFileBlob(repo, source, archiveRelative);
      if (committedBlob && !committedFileMatches(repo, source, archiveRelative, markdown, committedBlob)) addReason(reasons, 'release-note-not-in-source-commit', archiveRelative);
    } else if (!isDirectChild(repo, source, requested)) addReason(reasons, 'wrong-source-commit', `${requested} is neither archived source ${source} nor its release commit`);
