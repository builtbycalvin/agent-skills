...
export function parseReleaseNote(markdown, source = '<string>') {
  ...
  const recognized = new Set(["App Store What's New", 'TestFlight What to Test', 'Promotional Text', 'Evidence']); const counts = new Map(); for (const match of body.matchAll(/^## (.+?)\s*$/gm)) { const title = match[1].trim(); if (recognized.has(title)) counts.set(title, (counts.get(title) ?? 0) + 1); } for (const [title, count] of counts) if (count > 1) errors.push(`${title}: duplicate section`);
