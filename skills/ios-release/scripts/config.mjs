...
const RELEASE_NOTES_FIELDS = new Set(['archiveDirectory', 'sourceLocale', 'locales', 'tagPrefix', 'tone', 'promotionalText']);
...
function validateGroup(group, location, errors, ids) {
  ...
  if (group.id && ids.has(group.id)) errors.push(`${location}.id: duplicate group ID`); else if (group.id) ids.add(group.id);
}
function validateReleaseNotes(repo, notes, location, errors, missing) {
  if (!object(notes)) { errors.push(`${location}: expected an object`); missing.push(location); return; }
  unknown(notes, RELEASE_NOTES_FIELDS, location, errors);
  relativePath(repo, notes.archiveDirectory, `${location}.archiveDirectory`, errors, { rejectLocal: true });
  if (!nonEmpty(notes.sourceLocale) || !LOCALE.test(notes.sourceLocale)) errors.push(`${location}.sourceLocale: expected a valid locale`);
  if (!Array.isArray(notes.locales) || notes.locales.length === 0) errors.push(`${location}.locales: expected a non-empty array`);
  else {
    const seen = new Set();
    for (const [index, locale] of notes.locales.entries()) { if (!nonEmpty(locale) || !LOCALE.test(locale)) errors.push(`${location}.locales[${index}]: expected a valid locale`); if (seen.has(locale)) errors.push(`${location}.locales: duplicate locale`); seen.add(locale); }
    if (nonEmpty(notes.sourceLocale) && !seen.has(notes.sourceLocale)) errors.push(`${location}.sourceLocale: must be included in locales`);
  }
  optionalString(notes.tagPrefix, `${location}.tagPrefix`, errors); optionalString(notes.tone, `${location}.tone`, errors);
  if (!['preserve', 'suggest'].includes(notes.promotionalText)) errors.push(`${location}.promotionalText: expected preserve or suggest`);
}
function validateApp(repo, key, app, errors, missing) {
  ...
  validateReleaseNotes(repo, app.releaseNotes, `${location}.releaseNotes`, errors, missing);
...
function archiveSafe(repo, archive, errors) { const resolved = relativePath(repo, archive, 'releaseNotes.archiveDirectory', errors, { rejectLocal: true }); if (!resolved) return; const rel = path.relative(repo, resolved).split(path.sep); if (rel.some((part) => ['.git', '.asc', 'credentials', 'DerivedData', 'build', 'dist'].includes(part))) errors.push('releaseNotes.archiveDirectory: unsafe generated or credential location'); if (ignored(repo, path.relative(repo, resolved))) errors.push('releaseNotes.archiveDirectory: path is ignored'); }

export function parsePortableConfig(value, repo) { const errors = []; const missing = []; if (!object(value)) return { value: null, errors: ['$: expected an object'], missing }; unknown(value, ROOT_FIELDS, '$', errors); if (value.schemaVersion !== 2) errors.push('$.schemaVersion: expected 2'); if (!object(value.apps)) errors.push('$.apps: expected an object'); else { const keys = Object.keys(value.apps); if (value.defaultApp !== undefined && (!nonEmpty(value.defaultApp) || !Object.hasOwn(value.apps, value.defaultApp))) errors.push('$.defaultApp: unknown app'); const claims = selectorClaims(value.apps, errors); for (const key of keys) { if (!APP_KEY.test(key)) errors.push(`apps.${key}: invalid app key`); validateApp(repo, key, value.apps[key], errors, missing); } if (keys.length > 1 && value.defaultApp === undefined) missing.push('defaultApp'); }
...
export async function init(repoCandidate) { const repo = repoRoot(repoCandidate); const directory = path.join(repo, DIRECTORY); await mkdir(directory, { recursive: true }); const created = {}; for (const [relative, content, mode] of [[WHITELIST, IGNORE_CONTENT, 0o644], [CONFIG, json({ schemaVersion: 2, apps: {} }), 0o644], [LOCAL, json({ schemaVersion: 2, apps: {} }), 0o600]]) { const target = path.join(repo, relative); try { await writeFile(target, content, { encoding: 'utf8', flag: 'wx', mode }); created[relative] = true; } catch (error) { if (error.code !== 'EEXIST') throw error; created[relative] = false; } if (relative === LOCAL) await chmod(target, 0o600); }
...
export async function doctor(repoCandidate, selectedApp) { const repo = repoRoot(repoCandidate); const errors = []; const missing = []; await whitelistValid(repo, errors); const configPath = path.join(repo, CONFIG); const localPath = path.join(repo, LOCAL); let config; let local; try { config = JSON.parse(await readFile(configPath, 'utf8')); } catch (error) { errors.push(`${CONFIG}: ${error.code === 'ENOENT' ? 'file does not exist; run init' : `invalid JSON: ${error.message}`}`); } try { local = JSON.parse(await readFile(localPath, 'utf8')); } catch (error) { errors.push(`${LOCAL}: ${error.code === 'ENOENT' ? 'file does not exist; run init' : `invalid JSON: ${error.message}`}`); }
  if (config) { const parsed = parsePortableConfig(config, repo); errors.push(...parsed.errors); missing.push(...parsed.missing); for (const app of Object.values(config.apps ?? {})) if (object(app?.releaseNotes)) archiveSafe(repo, app.releaseNotes.archiveDirectory, errors); if (ignored(repo, CONFIG)) errors.push(`${CONFIG}: portable config must be visible to Git`); if (tracked(repo, LOCAL).length) errors.push(`${LOCAL}: local config must not be tracked`); if (selectedApp && object(config.apps)) { const claims = selectorClaims(config.apps, []); if (!claims.has(selectedApp.toLocaleLowerCase('en-US'))) errors.push(`--app: unknown app ${JSON.stringify(selectedApp)}`); } }
  if (local) { const parsed = parseLocalConfig(local); errors.push(...parsed.errors); for (const key of Object.keys(local.apps ?? {})) if (!config?.apps || !Object.hasOwn(config.apps, key)) errors.push(`${LOCAL}.apps.${key}: no matching portable app`); }
...
async function findArchiveCandidates(repo, appKey) { const candidates = new Set(); for (const relative of [`release-notes/ios/${appKey}`, `release-notes/${appKey}`, 'release-notes']) { const absolute = path.join(repo, relative); try { const info = await stat(absolute); if (info.isDirectory()) candidates.add(relative); } catch { } } return [...candidates]; }
...
export async function migrate(repoCandidate, mode = 'plan') { const repo = repoRoot(repoCandidate); const source = path.join(repo, V1); const backupPath = path.join(repo, BACKUP); let old; let sourcePresent = true; try { old = JSON.parse(await readFile(source, 'utf8')); } catch (error) { if (error.code !== 'ENOENT') return { command: 'migrate', state: 'incomplete', repo, errors: [`${V1}: invalid JSON: ${error.message}`], candidates: [] }; sourcePresent = false; try { old = JSON.parse(await readFile(backupPath, 'utf8')); } catch (backupError) { const detail = backupError.code === 'ENOENT' ? 'file does not exist' : `invalid JSON: ${backupError.message}`; return { command: 'migrate', state: 'incomplete', repo, errors: [`${V1}: file does not exist`, `${BACKUP}: ${detail}`], candidates: [] }; } }
  ...
  const appKeys = Object.keys(apps); const candidateMap = {}; const candidateErrors = []; for (const key of appKeys) { const candidates = await findArchiveCandidates(repo, key); if (candidates.length > 1) { candidateMap[key] = candidates; candidateErrors.push(`apps.${key}.releaseNotes.archiveDirectory: choose one candidate: ${candidates.join(', ')}`); } else if (!apps[key].releaseNotes) { const archiveDirectory = candidates[0] ?? (appKeys.length > 1 ? `release-notes/ios/${key}` : 'release-notes'); apps[key].releaseNotes = { archiveDirectory, sourceLocale: 'en-US', locales: ['en-US'], promotionalText: 'preserve' }; } }
  if (oldErrors.length) return { command: 'migrate', state: 'conflict', repo, errors: oldErrors, candidates: candidateMap }; if (candidateErrors.length) return { command: 'migrate', state: 'incomplete', repo, errors: candidateErrors, candidates: candidateMap }; const portable = { schemaVersion: 2, ...(old.defaultApp ? { defaultApp: old.defaultApp } : {}), apps }; const local = { schemaVersion: 2, apps: localApps }; const portableCheck = parsePortableConfig(portable, repo); const localCheck = parseLocalConfig(local); if (portableCheck.errors.length || localCheck.errors.length) return { command: 'migrate', state: 'conflict', repo, errors: [...portableCheck.errors, ...localCheck.errors], candidates: candidateMap }; const plan = { command: 'migrate', state: mode === 'plan' ? 'planned' : 'migrated', repo, writes: [WHITELIST, CONFIG, LOCAL], backup: BACKUP, removeExcludeRule: true, portable, local };
  if (mode === 'plan') return { ...plan, sourcePresent }; const directory = path.join(repo, DIRECTORY); await mkdir(directory, { recursive: true }); const expected = new Map([[WHITELIST, IGNORE_CONTENT], [CONFIG, json(portable)], [LOCAL, json(local)], [BACKUP, json(old)]]); for (const [relative, content] of expected) { try { const existing = await readFile(path.join(repo, relative), 'utf8'); let matches = existing === content; if (!matches && relative.endsWith('.json')) { try { matches = JSON.stringify(JSON.parse(existing)) === JSON.stringify(JSON.parse(content)); } catch { matches = false; } } if (!matches) return { command: 'migrate', state: 'conflict', repo, errors: [`${relative}: existing file conflicts with planned migration`], candidates: candidateMap }; } catch (error) { if (error.code !== 'ENOENT') throw error; } } for (const [relative, content, permissions] of [[WHITELIST, IGNORE_CONTENT, 0o644], [CONFIG, json(portable), 0o644], [LOCAL, json(local), 0o600]]) { const target = path.join(repo, relative); try { await stat(target); } catch { await atomicWrite(target, content, permissions); } } const excludePath = gitPath(repo, 'info/exclude'); try { const content = await readFile(excludePath, 'utf8'); const lines = content.split(/\r?\n/); const next = lines.filter((line) => line !== EXCLUDE_RULE).join('\n'); if (next !== content) await atomicWrite(excludePath, next, 0o644); } catch (error) { if (error.code !== 'ENOENT') throw error; }
