...
function configurationNamespaceGitlink(repo) { const records = execFileSync('git', ['-C', repo, 'ls-files', '--stage', '-z', '--', DIRECTORY], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).split('\0').filter(Boolean); return records.some((record) => { const separator = record.indexOf('\t'); return separator >= 0 && record.slice(0, separator).startsWith('160000 ') && record.slice(separator + 1) === DIRECTORY; }); }
...
function localTrackingBlocker(repo) { return tracked(repo, LOCAL).length ? `${LOCAL}: local config must not be tracked` : null; }
...
function parseArgs(argv) { const [command, ...rest] = argv; if (!['init', 'doctor', 'migrate', 'migrate-v1'].includes(command)) throw new Error('Usage: config.mjs <init|doctor|migrate> --repo PATH [--app KEY] [--plan|--apply]'); let repo = process.cwd(); let app; let mode = command === 'migrate-v1' ? 'apply' : command === 'migrate' ? 'plan' : undefined; for (let i = 0; i < rest.length; i += 1) { const flag = rest[i]; const value = rest[i + 1]; if (flag === '--repo' && value) { repo = value; i += 1; } else if (flag === '--app' && value && command === 'doctor') { app = value; i += 1; } else if (flag === '--plan' && command !== 'init') mode = 'plan'; else if (flag === '--apply' && command !== 'init') mode = 'apply'; else throw new Error(`Unknown or incomplete option: ${flag}`); } return { command: command === 'migrate-v1' ? 'migrate' : command, repo, app, mode }; }
